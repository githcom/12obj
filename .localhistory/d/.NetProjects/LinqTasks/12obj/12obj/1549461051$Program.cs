...
namespace _12obj
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            }).GroupBy(e => e.school, (k, g) => new {school = k, student = /*g.Select(r => r.student)*/g.Select(r => r.student.OrderBy(t => t)), studCount = g.Count()/*, year = g.OrderBy(r => r.year)*//*.First()/* g.Select(r => r.year)*/ })/*.Select(e => e.)*//*OrderBy(e => e.school).Select(e => e.school + " " + e.studCount + " " + e.student.First())*/;

            var res2 = res.Max(e => e.studCount);

            var res3 = res.Where(e => e.studCount == res2)/*.OrderBy(e => e.student).ThenBy(e => e.school)*/.Select(e => e.student.First() + " " + e.school  );
