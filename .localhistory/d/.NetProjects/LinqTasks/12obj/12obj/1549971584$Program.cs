    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            var res113 = res112.GroupBy(e => (e.flat - 1) / 4,
                (k, g) => new { floor = ++floor1, flatAll = g.Select(r => r.flat), elem = g.Select(r => r.e), flat = g.Select(r => r.e.flat), debt = g.Select(r => r.e.debt).DefaultIfEmpty() });
            ...
            var res6 = res113.GroupJoin(res213, q => q.floor, w => w.floor,
                (a1, a2) => new {floor = a1.floor, debt = a1.debt /*.Concat(a2.Select(t => t.debt))*/}).SelectMany(e => e.debt, (ee, d) => (ee.floor != 0 ? ee.floor : 0) + " " + (d != 0f ? d : 0f));
