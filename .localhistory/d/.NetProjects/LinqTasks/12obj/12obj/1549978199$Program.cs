    ...
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            var res113 = res112.GroupBy(e => (e.flat - 1) / 4,
                (k, g) => new { /*floor = ++floor1,*/ flatAll = g.Select(r => r.flat), elem = g.Select(r => r.e), flat = g.Select(r => r.e.flat), debt = g.DefaultIfEmpty().Select(r => r.e.debt).DefaultIfEmpty() }).Select((e, i) => new { floor = i + 1, flatAll = e.flatAll.DefaultIfEmpty(), elem = e.elem.DefaultIfEmpty(), flat = e.flat.DefaultIfEmpty(), debt = e.debt.DefaultIfEmpty() });
            ...
            var res213 = res212.GroupBy(e => (e.flat - 1) / 4,
                (k, g) => new { /*floor = ++floor2,*/ flatAll = g.Select(r => r.flat), elem = g.Select(r => r.e), flat = g.Select(r => r.e.flat), debt = g.Select(r => r.e.debt).DefaultIfEmpty() }).Select((e, i) => new { floor = i + 1, flatAll = e.flatAll, elem = e.elem, flat = e.flat, debt = e.debt});
            ...
            var res5 = res113.Join(res213, q => q.floor, w => w.floor, (a1, s2) => new { floor = a1.floor, debt1 = a1.debt.DefaultIfEmpty().Concat(s2.debt.DefaultIfEmpty())}) .Select(r => r.floor)  /*.SelectMany(e => e.debt1, (q, w) => q.floor + " " + w)*/;
